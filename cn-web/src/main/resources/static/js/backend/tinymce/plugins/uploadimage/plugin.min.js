/**
 * tinymce plugin
 * Created by jerry on 16/8/5.
 */
tinymce.PluginManager.add('uploadimage', function (editor) {

    function selectLocalImages() {
        var dom = editor.dom;
        var input_f = $(
            '<input type="file" name="file" accept="image/jpg,image/jpeg,image/png,image/gif" multiple="multiple">' +
            '<input type="hidden" name="token" value="' + uploadToken + '">' +
            '<input type="hidden" name="key" value="photo2/' + (Math.random().toString().substr(3)) + '">'
        );
        input_f.on('change', function () {
            var form = $("<form/>",
                {
                    action: editor.settings.images_upload_url, //设置上传图片的路由，配置在初始化时
                    style: 'display:none',
                    method: 'post',
                    enctype: 'multipart/form-data'
                }
            );
            form.append(input_f);

            //ajax提交表单
            form.ajaxSubmit({
                beforeSubmit: function () {
                    return true;
                },
                success: function (json) {
                    if (json && json.key) {
                        var newNode = editor.getDoc().createElement ("img");  // create img node
                        newNode.src='http://assets.duoec.com/' + json.key;
                        var range = editor.selection.getRng();
                        range.insertNode(newNode);
                    }
                }
            });
        });

        input_f.click();
    }

    editor.addCommand("mceUploadImageEditor", selectLocalImages);

    editor.addButton('uploadimage', {
        icon: 'image',
        tooltip: '上传图片',
        onclick: selectLocalImages
    });

    editor.addMenuItem('uploadimage', {
        icon: 'image',
        text: '上传图片',
        context: 'tools',
        onclick: selectLocalImages
    });
});