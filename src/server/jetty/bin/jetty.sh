...
#####################################################
JETTY_PORT=`sed '/jetty.http.port/!d;s/.*=//' $JETTY_HOME/conf/common.properties | tr -d '\r'`
if [ "$JETTY_PORT" ] 
then
...
# 增加配置文件路径指定参数.
#####################################################
JAVA_OPTIONS+=("-DconfigRootPath=$JETTY_HOME/conf")
...
# 增加logback配置文件路径指定参数.
#####################################################
JAVA_OPTIONS+=("-Dlogback.configurationFile=$JETTY_HOME/conf/logback.xml")

#####################################################
# In fact, this is what happens when you do a hot redeploy of a webapp.
#####################################################
JAVA_OPTIONS+=("-XX:+CMSClassUnloadingEnabled -XX:+UseConcMarkSweepGC")

#####################################################
# 不永久缓存dns
#####################################################
JAVA_OPTIONS+=("-Dsun.net.inetaddr.ttl=3 -Dsun.net.inetaddr.negative.ttl=3")
